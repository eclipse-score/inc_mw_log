...
using namespace amp::literals::string_view_literals;
...
const amp::string_view kContext{"aCtx"};
...
const amp::string_view kStringView{"Hello World"_sv};
...
class CompositeRecorderFixture : public ::testing::Test
{
  public:
    ...
    void AddRecorder(std::unique_ptr<Recorder> recorder) noexcept { recorders_.emplace_back(std::move(recorder)); }

    void CreateAllAvailableRecorders(amp::callback<std::unique_ptr<Recorder>(size_t)> create_recorder) noexcept
    {
...
TEST_F(CompositeRecorderFixture, StartRecordWithNoSlotAvailableShallDropRecorder)
{
    ...
    CreateAllAvailableRecorders([](std::size_t /*recorder*/) {
        auto mock_recorder = std::make_unique<RecorderMock>();
        EXPECT_CALL(*mock_recorder, StartRecord(kContext, kLogLevel)).WillOnce(Return(amp::optional<SlotHandle>{}));

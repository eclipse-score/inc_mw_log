...
void TerminateBuffer(LinearControlBlock& control_block, const Length offset, const Length length) noexcept
{
    ...
    if (DoBytesFitInRemainingCapacity(control_block.data, offset, GetLengthOffsetBytes()))
    {
        ...
        // 
        const auto src_span = amp::span<const Byte>{static_cast<const Byte*>(static_cast<const void*>(&length)), sizeof(length)};
        // clang-format on
    ...
    // which within uint64.
    // 
    amp::ignore = control_block.written_index.fetch_add(length + GetLengthOffsetBytes());
}

amp::optional<Length> CheckAndGetAcquireOffset(LinearControlBlock& control_block,
                                               const Length length,
...
amp::optional<AcquiredData> WaitFreeLinearWriter::Acquire(const Length length) noexcept
{

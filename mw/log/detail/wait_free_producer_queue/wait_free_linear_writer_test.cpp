...
TEST(WaitFreeLinearWriter, EnsureAtomicRequirements)
{
    ...
    bmw::mw::log::detail::LinearControlBlock control_block{};
...
TEST(WaitFreeLinearWriter, WriteBufferFullShouldReturnExpectedData)
{
    ...
    bmw::mw::log::detail::LinearControlBlock control_block{};
    control_block.data = amp::span<bmw::mw::log::detail::Byte>(buffer.data(), static_cast<std::int64_t>(buffer.size()));

    bmw::mw::log::detail::WaitFreeLinearWriter writer{control_block};
    ...
    for (auto i = 0u; i < kNumberOfWriterThreads; i++)
    {
        threads[i] = std::thread([i, kThreadAcquireFactor, &writer]() {
            ...
            {
                acquired_data.data()[payload_index] = static_cast<bmw::mw::log::detail::Byte>(payload_index);
            }
    ...
    auto reader = bmw::mw::log::detail::CreateLinearReaderFromControlBlock(control_block);
    ...
    while (true)
    {
        ...
        for (auto payload_index = 0u; payload_index < read_result.value().size(); payload_index++)
        {
            ASSERT_EQ(read_result.value().data()[payload_index],
                      static_cast<bmw::mw::log::detail::Byte>(payload_index));
        }
    }
}
...
TEST(WaitFreeLinearWriter, TooManyConcurrentWriterShouldReturnEmpty)
{
    ...
    bmw::mw::log::detail::LinearControlBlock control_block{};
    control_block.data = amp::span<bmw::mw::log::detail::Byte>(buffer.data(), static_cast<std::int64_t>(buffer.size()));
    control_block.number_of_writers.store(bmw::mw::log::detail::GetMaxNumberOfConcurrentWriters());
...
TEST(WaitFreeLinearWriter, BufferSizeExceededShouldReturnEmpty)
{
    ...
    bmw::mw::log::detail::LinearControlBlock control_block{};
    control_block.data = amp::span<bmw::mw::log::detail::Byte>(buffer.data(), static_cast<std::int64_t>(buffer.size()));
    control_block.acquired_index.store(static_cast<bmw::mw::log::detail::Length>(control_block.data.size()));
...
TEST(WaitFreeLinearWriter, BufferSizeExceededUpperLimitShouldReturnEmpty)
{
    ...
    bmw::mw::log::detail::LinearControlBlock control_block{};
    control_block.data = amp::span<bmw::mw::log::detail::Byte>(buffer.data(), static_cast<std::int64_t>(buffer.size()));
    control_block.acquired_index.store(bmw::mw::log::detail::GetMaxLinearBufferCapacityBytes());
...
TEST(WaitFreeLinearWriter, FailedAcquireShouldTerminateBuffer)
{
    ...
    bmw::mw::log::detail::AcquiredData acquired_data{};

    bmw::mw::log::detail::WaitFreeLinearWriter writer(
        control_block, [i = 0, &acquired_data](bmw::mw::log::detail::WaitFreeLinearWriter& writer_callback) mutable {
    ...
    ASSERT_FALSE(writer.Acquire(bmw::mw::log::detail::GetLengthOffsetBytes()).has_value());
    ...
    const auto expected_acquired_index =
        bmw::mw::log::detail::GetLengthOffsetBytes() + 2 * bmw::mw::log::detail::GetLengthOffsetBytes();
...
TEST(WaitFreeLinearWriter, FailedAcquireWithNoFreeSpaceShouldNotTerminateBuffer)
{
    ...
    bmw::mw::log::detail::WaitFreeLinearWriter writer(
        control_block, [i = 0, &acquired_data](bmw::mw::log::detail::WaitFreeLinearWriter& writer_callback) mutable {
            ...
            if (i++ == 0)
            {
                auto acquire_result = writer_callback.Acquire(bmw::mw::log::detail::GetLengthOffsetBytes());
                if (acquire_result.has_value() == false)
                {
                    std::abort();
                }
    ...
    // length.
    const auto expected_acquired_index =
        2 * bmw::mw::log::detail::GetLengthOffsetBytes() + 2 * bmw::mw::log::detail::GetLengthOffsetBytes();
    ASSERT_EQ(control_block.acquired_index, expected_acquired_index);
    ASSERT_EQ(control_block.written_index, control_block.acquired_index);
    ...
    auto read_result = reader.Read();
    ASSERT_TRUE(read_result.has_value());
    ASSERT_EQ(read_result.value().size(), bmw::mw::log::detail::GetLengthOffsetBytes());
    ASSERT_FALSE(reader.Read().has_value());
}
...
TEST(WaitFreeLinearWriter, AcquireMoreThanMaximumShouldFail)
{
    ...
    ASSERT_FALSE(writer.Acquire(bmw::mw::log::detail::GetMaxAcquireLengthBytes() + 1).has_value());
}

...
#ifndef PLATFORM_AAS_MW_LOG_DETAIL_VERBOSE_PAYLOAD_H
...
using ReserveCallback = amp::callback<std::size_t(amp::span<Byte>)>;
...
class VerbosePayload final
{
  public:
    ...
    ///
    /// \param callback function with signature std::size(*f)(amp::callback<Byte>) used to fill provided buffer
    ...
    std::size_t Put(const ReserveCallback callback, const std::size_t reserve_size = 0) noexcept;

    amp::span<const std::uint8_t> GetSpan() const noexcept;
...
#endif  // PLATFORM_AAS_MW_LOG_DETAIL_VERBOSE_PAYLOAD_H

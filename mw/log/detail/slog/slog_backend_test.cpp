...
struct SlogBackendFixture : ::testing::Test
{
    void SetUp() override
    {
        slog2_mock_ = amp::pmr::make_unique<bmw::os::qnx::MockSlog2>(amp::pmr::get_default_resource());
  ...
  protected:
                         ...
                         const std::uint16_t& code = kDefaultCode)
    {
        ...
        log_entry.ctx_id = LoggingIdentifier(amp::string_view(ctx_id));
    ...
    amp::pmr::unique_ptr<bmw::os::qnx::MockSlog2> slog2_mock_{};
    bmw::os::qnx::MockSlog2* slog2_mock_raw_ptr_;
};
...
TEST_F(SlogBackendFixture, SlogRegisterShouldHandleError)
{
    ...
    EXPECT_CALL(*slog2_mock_raw_ptr_, slog2_register(_, _, _))
        .Times(1)
        .WillOnce(Return(amp::make_unexpected<bmw::os::Error>(bmw::os::Error::createFromErrno())));

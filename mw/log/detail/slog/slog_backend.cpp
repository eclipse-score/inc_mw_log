...
SlogBackend::SlogBackend(
    ...
    amp::pmr::unique_ptr<bmw::os::qnx::Slog2> slog2_instance /* = bmw::os::qnx::Slog2() */) noexcept
    : Backend::Backend(),
...
amp::optional<SlotHandle> SlogBackend::ReserveSlot() noexcept
{
    const auto& slot = buffer_.AcquireSlotToWrite();
    if (slot.has_value())
    {
        if (slot.value() < std::numeric_limits<SlotIndex>::max())
        {
...
// 
    ...
    amp::ignore = slog2_instance_->slog2f(slog_buffer_,
                                          slog2,

...
#include "platform/aas/mw/log/detail/backend_mock.h"
...
class TextRecorderFixtureWithLogLevelCheck : public testing::Test
{
  ...
  protected:
    ...
    const amp::string_view context_id_ = "DFLT";
...
TEST_F(TextRecorderFixtureWithLogLevelCheck, WillObtainEmptySlotsWhenNoSlotsReserved)
{
    ...
    ON_CALL(*backend_mock, ReserveSlot()).WillByDefault(Invoke([]() -> amp::optional<SlotHandle> { return {}; }));
...
TEST_F(TextRecorderFixture, TooManyArgumentsWillYieldTruncatedLog)
{
    ...
    const amp::string_view message{"byte"};
...
TEST_F(TextRecorderFixture, TooLargeSinglePayloadWillYieldTruncatedLog)
{
    ...
    recorder_->Log(SlotHandle{}, amp::string_view{vec.data(), too_big_data_size});
    recorder_->Log(SlotHandle{}, amp::string_view{"xxx"});
...
TEST_F(TextRecorderFixture, LogStringView)
{
    ...
    recorder_->Log(SlotHandle{}, amp::string_view{"Hello world"});
}
...
TEST(TextRecorderTests, TextRecorderShouldClearSlotOnStart)
{
    ...
    const auto payload = amp::string_view{"Hello world"};

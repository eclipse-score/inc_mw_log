...
amp::optional<SlotHandle> FileRecorder::StartRecord(const amp::string_view context_id,
...
void FileRecorder::Log(const SlotHandle& slot, const amp::string_view data) noexcept
{
    LogData(slot, *backend_, data);
}
...
bool FileRecorder::IsLogEnabled(const LogLevel& log_level, const amp::string_view context) const noexcept
{

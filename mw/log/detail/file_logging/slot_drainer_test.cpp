...
class SlotDrainerFixture : public ::testing::Test
{
  public:
    void SetUp() override
    {
        unistd_ptr_ = amp::pmr::make_unique<bmw::os::UnistdMock>(amp::pmr::get_default_resource());
  ...
  protected:
    ...
    amp::pmr::unique_ptr<::bmw::os::UnistdMock> unistd_ptr_{};
...
TEST_F(SlotDrainerFixture, TestOneWriteFileFailurePath)
{
    ...
    EXPECT_CALL(*raw_message_builder_mock_, GetNextSpan)
        ...
        .WillOnce(
            Return(amp::span<const std::uint8_t>(data_table_, sizeof(data_table_))));  //  actual data to be written
    ...
    EXPECT_CALL(*unistd_mock_, write(file_descriptor_, _, _))
        .WillRepeatedly(Return(amp::unexpected<bmw::os::Error>(bmw::os::Error::createFromErrno(EIO))));
...
TEST_F(SlotDrainerFixture, IncompleteWriteFileShouldMakeFlushSpansReturnWouldBlock)
{
    ...
    EXPECT_CALL(*unistd_mock_, write(file_descriptor_, _, _))
        .WillOnce([](int, const void*, size_t) { return 1; })
        .WillOnce([](int, const void*, size_t) { return 1; });
...
TEST_F(SlotDrainerFixture, TestOneSlotOneSpan)
{
    ...
    EXPECT_CALL(*unistd_mock_, write(file_descriptor_, data_table_, sizeof(data_table_)))
        .WillOnce([](int, const void*, size_t) { return sizeof(data_table_); });
